...
if __name__ == '__main__':
    ...
    parser.add_argument("iday_new")
    parser.add_argument("istart_new")
    parser.add_argument("iend_new")
    ...
    iday = int(args.iday_new)
    istart = int(args.istart_new)
    iend = int(args.iend_new)
    ...
    with nc.Dataset(infile1,'a') as infile1:
        with nc.Dataset(infile2,'a') as infile2:
            for vname in invars:
                ...
                infile1.variables[vname][:] = (indata2[:]-indata1[:])*(iday-istart)/(iend-istart)+indata1[:]
